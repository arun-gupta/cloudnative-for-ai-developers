...
Cold start hurts most in three situations: a sudden spike in requests hits your model endpoint and capacity needs to expand fast; your endpoint was idle and the first request after a quiet period has to wait for everything to load; or you deploy a new model version and users hit errors or timeouts during the switchover.
